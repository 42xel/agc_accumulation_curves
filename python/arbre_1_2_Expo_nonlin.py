...
def sqrtrec (x, y, n):
    ...
    if x + t == x or n== 0:
        ...
        x += t
        n -= 1
        return sqrtrec (x, y, n)

def sqrt(y):
    if y <= 0:
        return 0
    else:
        return sqrtrec(y, y, 100)
...
def machine(depth, u, p, di):
        """choses a step in function of the parameter a and c and xm
        px is the value of the parabola at x (m1 stands for minus one)
        px = y0 * a^x + c"""
        
        a = (p.p1-p.p0)/(p.p0-p.pm1)
        if a == 1:
            y0 = 0
            c = p.p0
        elif a == 0:
...
#PM1 = g(-1), P0 = g(0), P1 = g(1), where g is the function to draw, here an exponential function.
#domaine de validité :
        #must all be greater than 2 and monotonous. TOFINISH
PM1 = 2
P0 = 7/2
P1 = 4
